...
We're using Sinatra without any additional packages beyond what was originally configured.  Simply run the following to start the server (WEBrick default port :4567).
...
* **public/css/default.css** - basic styling
* **public/js/main.js** - jQuery AJAX handling of submission

## Requirements Checklist

#### Respond to a client HTTP request

-[x] A random body of text. Some sample texts are included.

> Continue to use the text/[0-9]+ sample texts.

-[x] A list of some but not all words in that body of text (exception: texts with only one unique word should use an empty list)

> Exclusion list generated by **captcha.rb#generate_exclusion_array**.  Takes a source_text along with an optional size of the exclusion_array to generate.  If a size is specified, we return an exclusion list of up to the lesser of that size or one less than the total number of unique words.  If no size specified (default case), we return an exclusion list of at least one size smaller than the total unique word count.  The maximum size of the exclusion list is the smaller of 1/2 the total number of unique words or 5, since we don't want too many words to exclude to make the task tedious.
>
> number_of_words_to_exclude is a helper function that figures out how big the exclusion array should be -- the intricacies of which are detailed above.
>
> generate_word_array is a helper function that creates a unique word array from text.  To standardize each word, they are all downcased and stripped of punctuation.  We don't handle the edgecases of single quotes or dashes that could require specialized handling.

#### Receive a client HTTP request with at least the following in the request

-[x] A body of text

> Read from the http data: #captcha .text

-[x] A list of some but not all words in that body of text (exception: texts with only one unique word should have an empty list)

> Read from the http data: #captcha .exclude

-[x] A count of the frequency of every word in the body of text excluding words from the list

> User input requested for the word count input[name="word_count"]

#### Bonus

-[ ] Cheating Protection

> Stubbed out using a unique hash to identify source_text, but not yet implemented.

-[x] Stateless/Idempotent

> Implementation satisfies this requirement, there are no prior assumptions about state.  The POST request data is cobbled together from the DOM, so the main vulnerability would be if the user maniupated the DOM through developer tools (or other such techniques).
